...
class DatetimeTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testDateTimeImmutableTimestampOffsetCalculation() {
        $datetime = '2022-01-14 18:00';
        $datetimeUtc = new \DateTimeImmutable($datetime, new \DateTimeZone('UTC'));
        ...
        $this->assertLessThan(
            ...
            $datetimeUtc->getTimestamp() + $datetimeUsNy->getOffset()
        );
        $this->assertGreaterThan(
            $datetimeUtc->getTimestamp(),
            $datetimeUtc->getTimestamp() + $datetimeEuMo->getOffset()
        );
        $this->assertGreaterThan(
            $datetimeUtc->getTimestamp(),
            $datetimeUtc->getTimestamp() + $datetimeEuZg->getOffset());
    }

    public function testDateTimeImmutableTimestampOffsetDifference() {
        $datetime = '2022-01-14 18:00';
        $datetimeUtc = new \DateTimeImmutable($datetime, new \DateTimeZone('UTC'));
        ...
        $this->assertEquals($datetimeUtc->getTimestamp(), $datetimeUsNy->getTimestamp() + $datetimeUsNy->getOffset());
        $this->assertEquals($datetimeUtc->getTimestamp(), $datetimeEuMo->getTimestamp() + $datetimeEuMo->getOffset());
        $this->assertEquals($datetimeUtc->getTimestamp(), $datetimeEuZg->getTimestamp() + $datetimeEuZg->getOffset());
    }
}
